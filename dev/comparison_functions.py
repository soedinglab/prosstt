...
def plot(real_name, real, sim, alpha=0.5):
    ...
    # mean ~ variance
    ax[1][0].set_title("mean ~ variance")
    ax[1][0].set_xlabel("log2(x+1) of avg. gene expr.")
    ax[1][0].set_ylabel("log2(x+1) of gene expr. variance")
    s1 = ax[1][0].scatter(np.log2(real_genes.loc['means']+1),
                        np.log2(real_genes.loc['var']+1),
                        label=real_name, alpha=alpha)
    s2 = ax[1][0].scatter(np.log2(sim_means+1),
                        np.log2(sim_vars+1),
                        label="prosstt", alpha=alpha)

    # mean gene expression
    ax[1][1].set_title("avg. gene expr.")
    ax[1][1].set_xlabel("log2(x+1) avg. expr. data")
    ax[1][1].set_ylabel("log2(x+1) avg. expr. simulation")
    ax[1][1].scatter(np.log2(real_genes.loc['means'] + 1),
