...
def save_files(job_id, save_dir, X, labs, brns, uMs, gene_scale):
...
def main(job_id, save_dir):
	...
	t = tree.Tree(topology=top, time=br_lengths, branches=num_branches,
		branch_points=num_brpoints, modules=br_compl)

	sample_time = np.arange(0, t.get_max_time())

	Ms = None
	while not sim.are_lengths_ok(Ms):
	    uMs, Ws = sim.simulate_branching_data(G, t)
	...
	save_files(job_id, save_dir, X, labs, brns, uMs, gene_scale)
...
if __name__ == "__main__":
	...
	main(options.job, options.outdir)