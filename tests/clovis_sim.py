...
def main(job_id, save_dir):
    ...
    G = 500
    alpha = np.exp(random.normal(np.log(0.2), 0.5))
    beta = 0
    while(beta < 1):
        beta = np.exp(random.normal(np.log(2), 0.5))


    # the topology is non-negotiable since this is a single bifurcation
    num_branches = 7
    num_brpoints = 3
    top = [[0,1], [0,2], [2,3], [2,4], [4,5], [4,6]]

    br_lengths = np.array([60]*7) # pseudotime smaller than 20
    # makes little sense as that usually does not give the random walk enough time
    # to create useful variation
    br_compl = random.random_integers(20, 40, num_branches) # more than 50 components is too much
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Generate a limited single bifurcation for Clovis.')
